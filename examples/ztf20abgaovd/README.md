...
- **R_V = 3.56 ± 0.89** (median ± star-to-star MAD, 775 stars with A_V ≥ 0.5; per-star
  errors 0.4–0.6, so ~±0.04 statistical on the median). By A_V bin: 3.50 (0.5–0.7,
  N 441), 3.63 (0.7–1.0, N 263), 3.79 (1–2, N 70). The per-star distribution piles up at
...
- **APOGEE check** (45 stars, mostly giants, log g ≈ 2.8): T_eff(fit) − T_eff(ASPCAP) =
  −108 K (MAD 147); log g +0.06; [Fe/H](DESI) − [Fe/H](APOGEE) = 0.00. Locking those
  stars to ASPCAP moves their A_V 0.50 → 0.68 (giants) / 0.49 → 0.59 (dwarfs, −65 K) and
  R_V 3.0 → 3.2 / 3.3 → 3.4.
...
- The empirical template corrections cover dwarfs only; the distant stars (D > 4 kpc)
  are giants on uncorrected templates, which is the likely cause of the rise in A(D)
  beyond 5 kpc. Calibrating giants (nearby APOGEE giants with XP, same machinery) is the
  natural next step.
