...
stage from a local cache instead of the ~2 h archive fetch. Numbers are from the v0.5.0
stack: the 900 Å–6 µm model cache with the empirical dwarf-template corrections and the
tile-based, saturation-cut PS1 fetch; v0.4.0 gave R_V = 2.84 ± 0.48 (946 stars) and the
v0.2–0.3 run on the 2500–25000 Å cache 2.88 ± 0.53 (986 stars), clump column 3.28–3.33.)

## Result

- **R_V = 2.80 ± 0.46** (median ± MAD, 895 stars with A_V ≥ 2).
- **Red-clump anchor:** E(J−Ks) = 0.52 from 1,820 clump-window stars at Ks = 12.65,
  giving A_V = 3.27 ± 0.61 (A_Ks = 0.31) at D_RC = 6.2 kpc.
- **A_I(D)** (A_I/A_V = 0.579 under the measured law):
  ...
  | 4 | 1.16 | | 200 | |
  | 5 | 1.26 | | 84 | |
  | ≥ 7 | 1.90 | | 0 | bridged to the clump column |
