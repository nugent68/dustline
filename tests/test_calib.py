...
def _corr():
    ...
    fit = pd.DataFrame(dict(teff_spec=np.r_[np.full(n // 2, 4600.0), np.full(n // 2, 5600.0)],
                            feh_spec=np.full(n, 0.0), logg_spec=np.full(n, 4.5),
...
def test_aggregate_bins_and_normalisation():
    ...
    assert corr["n"][t_cool, z, 0] == 30 and corr["n"][t_hot, z, 0] == 30   # class 0 = dwarfs
    assert corr["n"][:, :, 1].sum() == 0                                    # no giants in the input
    R = corr["ratio"][t_cool, z, 0]
    ...
    assert abs(np.interp(400, wave, corr["ratio"][t_hot, z, 0]) - 1.0) < 0.01
    assert corr["band_dm"][t_cool, z, 0, 0] == pytest_approx(0.10, 0.01)
    assert corr["n"][0, z, 0] == 0                               # empty bin -> no correction
...
def test_apply_only_dwarfs_in_populated_bins():
    ...
    assert list(used) == [True, False, False, True]            # giant (no giant class data) and empty bin untouched
