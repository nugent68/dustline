...
def test_priors_present_and_consistent(fit):
    ...
    assert set(np.unique(fit.mh_best)) <= {-0.5, 0.0, 0.5}
...
def test_column_regression(fit):
    ...
    assert c["clean"]["n"] == pytest.approx(150, abs=10)
    assert c["clean"]["av"] == pytest.approx(0.086, abs=0.02)
    assert c["clean"]["av_mad"] < 0.07
    assert c["teff_cool"]["av"] > 0.15                        # the cool-star systematic
    assert c["clean_with_spec_prior"]["av_mad"] < 0.045
