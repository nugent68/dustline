...
column is the DECam Y band).  Data Lab column names (checked 2026-09-21):
mean_mag_<b>, err_<b> (not err_mag_<b>), nmag_<b>, fracflux_avg_<b>.
"""
...
def fetch(ws: Workspace, gaia: pd.DataFrame, force: bool = False) -> pd.DataFrame:
    ...
    cols = ["ra", "dec", "posstdev"] + \
        [f"mean_mag_{b}" for b in _BANDS] + [f"err_{b}" for b in _BANDS] + \
    ...
    for b, name in _BANDS.items():
        ...
        res[f"magerr_{name}"] = np.where(
            good, np.sqrt(p[f"err_{b}"].values ** 2 + SYS_FLOOR ** 2), np.nan)
